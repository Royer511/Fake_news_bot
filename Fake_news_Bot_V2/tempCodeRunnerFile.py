...
async def checknews(ctx, link: str):
    ...
    # Check if the URL is from a social media domain
    ...
    # Check if the URL is from a blacklisted website
    if is_blacklisted(domain):
        await ctx.send(f"⚠️ Warning: The domain {domain} is known for hosting misleading or false information.")
        return

    # If the URL passed both checks, send a message indicating it appears to be okay
    await ctx.send("✅ This source appears to be okay, but always stay vigilant.")
...
async def on_message(message):
    ...
    should_process_commands = True  # Flag to decide if further command processing should occur
...
async def summary(ctx, url: str):
    try:
        ...
        summarized_text = summarizer(article_text, max_length=1200, min_length=100, do_sample=False)
